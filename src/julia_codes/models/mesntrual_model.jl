...
# ╔═╡ 612ad38a-9fc0-47de-b809-824fdc488ec6
begin
	using Pluto
	using PlutoUIExtra
...
begin
	# Hypothalamus parameters
	outside_ovary_params = DataFrame(
	    Parameter = ["V_0_LH", "V_1_LH", "Km_LH", "Ki_LH_P", "k_LH", "c_LH_P", "c_LH_E", "v", "α_LH", "Ki_FSH_Inh", "w", "k_FSH", "c_FSH_P", "c_FSH_E", "V_FSH", "α_FSH"],
	    Value = [550.03, 3329.19, 136.05/1000, 6.78, 0.9661, 1.98, 1000*0.0060, 2.5, 14, 16.83, 9.21, 14.59, 52.31, 0.0151*1000^2, 294.90, 8.21],
	    Type = fill("Outside", 16)
	)
	
	# Ovary parameters
	ovary_params = DataFrame(
	    Parameter = ["b", "c_1", "q", "c_2", "c_3", "c_4", "d_1", "d_2", "k_1", "k_2", "k_3", "k_4", "α", "γ"],
	    Value = [0.0453, 0.1036, 5.11, 0.0577, 0.0170, 1.14, 0.7537, 0.6866, 0.6699, 0.6388, 0.9191, 1.88, 0.9505, 0.1615],
	    Type = fill("Ovary", 14)
	)
	...
	# Initial values
	initial_values = DataFrame(
		Parameter = ["RP_LH", "LH", "RP_FSH", "FSH", "RcF", "GrF", "DomF", "Sc_1", "Sc_2", "Lut_1", "Lut_2", "Lut_3", "Lut_4"],
		Value = [167.57, 11.81, 14.48, 11.41, 2.10, 4.12, 0.46, 1.06, 1.67, 4.16, 13.03, 16.48, 10.29],
		Type = fill("Initial", 13)
	)
	
	# Absorption parameters
	control_params = DataFrame(
	    Parameter = ["u1", "u2", "μ"],
	    Value = [zeros(28), zeros(28), 9/24],
	    Type = fill("Control", 3)
	)
	
	# Concatenate all parameters
	all_params = vcat(outside_ovary_params, ovary_params, auxiliary_params, circadian_params, abs_params, initial_values, control_params)

	using CSV
	filepath = joinpath("..","..","..","res")
	CSV.write(joinpath(filepath, "parameter_values.csv"), all_params)
end
...
begin
	param_names = all_params.Parameter
	param_values = all_params.Value
	
	include("dde_model.jl")
end

# ╔═╡ 57b43519-5053-45e4-9f94-3d96cc990ea6
md"""
...
\frac{d\text{DomF}}{dt} &= c_3\text{LH}\text{GrF}-c_4\text{LH}^\gamma\text{DomF}\\
\frac{d\text{Sc}_1}{dt}&=c_4\text{LH}^\gamma\text{DomF}-d_1\text{Sc}_1\\
\frac{d\text{Sc}_2}{dt}&=d_1\text{Sc}_1-d_2\text{Sc}_2\\
\frac{d\text{Lut}_1}{dt}&=d_2\text{Sc}_2-k_1\text{Lut}_1\\
\frac{d\text{Lut}_2}{dt}&=k_1\text{Lut}_1-k_2\text{Lut}_2\\
\frac{d\text{Lut}_3}{dt}&=k_2\text{Lut}_2-k_3\text{Lut}_3\\
\frac{d\text{Lut}_4}{dt}&=k_3\text{Lut}_3-k_4\text{Lut}_4
\end{align}$$

Auxiliary equations

$$\begin{align}
E_2&=(e_0+e_1\text{GrF}+e_2\text{DomF}+e_3\text{Lut}_4)(1+\theta_5\cos(2\pi(t-\theta_6)))+\eta_{E_2}C_E(t,\mathbf{u}_1,\mu)\\
P_4 &= (p_1\text{Lut}_3+p_2\text{Lut}_4)(1+\theta_7\cos(2\pi(t-\theta_8)))+\eta_{P_4}C_P(t,\mathbf{u}_2,\mu)\\
\text{Inh} &= h_0+h_1\text{DomF}+h_2\text{Lut}_2+h_3\text{Lut}_3
\end{align}$$

where $C_E(t,\mathbf{u}_1,\mu)=\sum\limits^{27}_{i=0}C_{E,i}(t,\mathbf{u}_1,\mu)$ and $C_P(t,\mathbf{u}_2,\mu)=\sum\limits^{27}_{i=0}C_{P,i}(t,\mathbf{u}_2,\mu)$. Assume the $\mu$ is determined for now.

$$\begin{align}
C_{E,i}&=\begin{cases}
            0 &t\in[i,i+\mu_i) \\
            \frac{F_{E_2}k_{a,E_2}}{V_D(k_{a,E_2}-k_{e,E_2})}\left(e^{-k_{e,E_2}(t-(i-1+\mu_i))}-e^{-k_{a,E_2}(t-(i-1+\mu_i))}\right)u_{1i} & t\in[i+\mu_i, i+1)
\end{cases}\\
C_{P,i}&=\begin{cases}
            0 &t\in[i,i+\mu_i) \\
            \frac{F_{P_4}k_{a,P_4}}{V_D(k_{a,P_4}-k_{e,P_4})}\left(e^{-k_{e,P_4}(t-(i-1+\mu_i))}-e^{-k_{a,P_4}(t-(i-1+\mu_i))}\right)u_{2i} & t\in[i+\mu_i, i+1)
\end{cases}
\end{align}$$

"""

# ╔═╡ 8215e400-d731-4857-82d8-fe2da3725aa4
md"""
# Packages
"""
...
md"## Reproduce the result"

# ╔═╡ 66caf0f3-1189-4d8e-ae11-97761449f3b5
md"### Parameter definitions"
...
begin
	...
	initial_condition_names = ["RP_LH", "LH", "RP_FSH", "FSH", "RcF", "GrF", "DomF", "Sc_1", "Sc_2", "Lut_1", "Lut_2", "Lut_3", "Lut_4"]
	
    x0, tspan, lags = setup_initial_conditions_time_info(param_values)
	tspan = (0,cycle_num*28.0)
end

# ╔═╡ 22ba5c37-4e26-441e-a214-f002b8e86f46
md"## Parameter slides"

# ╔═╡ 5958a5e3-4d89-4e16-a222-7a4b8f35b5ec
function set_params(names::Vector, values::Vector, par_name::String, loc::String)
	
	return PlutoUI.combine() do Child
		
		inputs = [
			md""" $(name): $(
				Child(name, Slider(0:value/4:2*value; default=value, show_value=true))
			)"""
...
function set_params_u(names::Vector, values::Vector, par_name::String, loc::String)
	...
	return PlutoUI.combine() do Child
        ...
        # Group inputs in pairs
        inputs = []
        i = 1
        while i <= length(names)/2
			# Add two sliders in one line
			push!(inputs, md""" $(names[i]): $(Child(names[i], Slider(0:values[i]/4:2*values[i]; default=values[i], show_value=true))) $(names[i+28]): $(Child(names[i+28], Slider(0:values[i+28]/4:2*values[i+28]; default=values[i+28], show_value=true))) """)
			i += 1
        end
...
@bind control_param set_params_u(vcat(["u1_" * string(i) for i in 1:28], ["u2_" * string(i) for i in 1:28]), vcat(30*ones(28), 2e3*ones(28)), "💊", "upper right")

# ╔═╡ 0bfcb59c-3be8-4965-b4d7-acd165584df4
function set_params_μ(directions::Vector, values::Vector, par_name::String, loc::String)
	
	return PlutoUI.combine() do Child
		
		inputs = [
			md""" $(name): $(
				Child(name, Slider(0:24; default=value, show_value=true))
			)"""
			
			for (name, value) in zip(directions, values)
		]
...
@bind control_param_μ set_params_μ(["μ"], [9], "🕘", "lower right")
...
begin
    ...
    # u1으로 시작하는 값들을 u1_values에 채우기
	u1_keys = filter(key -> startswith(string(key), "u1_"), collect(keys(control_param)))
    u1_keys = sort(u1_keys, by = x -> parse(Int, split(string(x), "_")[2]))
	for (i, key) in enumerate(u1_keys)
		u1_values[i] = control_param[key]
	end

    # u2로 시작하는 값들을 u2_values에 채우기
	u2_keys = filter(key -> startswith(string(key), "u2_"), collect(keys(control_param)))
    u2_keys = sort(u2_keys, by = x -> parse(Int, split(string(x), "_")[2]))
	...
	parameter_set6 = NamedTuple{(:u1,:u2,:μ)}((u1_values,u2_values,μ))
	nothing
end

# ╔═╡ 3a4cee59-2fb2-461e-9d7c-e719dde33f91
bar((0:27), u1_values; label="u₁", bar_width=1, alpha=1)

# ╔═╡ d459e902-1499-408e-b5da-99452f118ab2
bar((0:27), u2_values; label="u₂", bar_width=1, alpha=1, color=:red)

# ╔═╡ db5748ce-27ec-4e59-b9cc-c76d278d95a9
begin
	total_set = merge(parameter_set, parameter_set6)
	
	new_parameters = deepcopy(all_params)

	for (i, param) in enumerate(new_parameters.Parameter)
	    if haskey(total_set, Symbol(param))
	        new_parameters.Value[i] = total_set[Symbol(param)]
	    else
	        println("Warning: Parameter ", param, " not found in new_values.")
	    end
	end
end

# ╔═╡ 60c5a890-c8c0-4ec8-a913-c98acb9e1bbf
new_parameters

# ╔═╡ 66b5f913-a762-4f41-9e46-a4c057fb4ea4
md"""
## State graphs
"""

# ╔═╡ f931907d-b7d3-4f75-8626-6c7c2b055ca1
begin
	new_prob = DDEProblem(menstrual_cycle_dynamics!, x0, history_func, tspan, new_parameters.Value; constant_lags=lags)
	# solve DDE
	new_sol = solve(new_prob, MethodOfSteps(RK4()), dtmax=0.05)
end

# ╔═╡ 24cc95ec-0408-4b3f-99b3-90cb64719d24
begin
	# 결과 플로팅
	new_plots = []
	for (i, name) in enumerate(initial_condition_names)
	    p = plot(new_sol, vars=(0,i), title=name, xlabel="Time (days)", ylabel="Concentration", legend=false, linewidth=4)
	    push!(new_plots, p)
	end
	
	# E2, P4, Inh 계산 및 그래프 추가
	new_E2_values = []
	new_P4_values = []
	new_Inh_values = []
	
    new_E2_values, new_P4_values, new_Inh_values = calculate_auxiliary(new_sol, new_parameters.Value, new_sol.t)
	
	push!(new_plots, plot(new_sol.t, new_E2_values, title="E2", xlabel="Time (days)", ylabel="Concentration", legend=false, linewidth=4))
	push!(new_plots, plot(new_sol.t, new_P4_values, title="P4", xlabel="Time (days)", ylabel="Concentration", legend=false, linewidth=4))
	push!(new_plots, plot(new_sol.t, new_Inh_values, title="Inh", xlabel="Time (days)", ylabel="Concentration", legend=false, linewidth=4))

	# Concatenation
	new_final_plot = plot(new_plots..., layout=(4,4), size=(1200,1200))
end

# ╔═╡ 379aa47f-f8db-48d4-ba2d-bdbd90813660
new_parameters

# ╔═╡ 82281cb0-d22f-4a1a-aa8d-187df008916d
begin
	main_plots = []
	
	push!(main_plots, plot(new_sol.t, new_P4_values, title="P4", xlabel="Time (days)", ylabel="Concentration", legend=false, linewidth=4))
	
	push!(main_plots, plot(new_sol, vars=(0,4), title="FSH", xlabel="Time (days)", ylabel="Concentration", legend=false, linewidth=4))
	
	main_final_plot = plot(main_plots..., layout=(1,2), size=(800,300))
end
...
manifest_format = "2.0"
project_hash = "fd58f9565b4c52c1abc4889783f0339eeab4da53"
...
[[deps.JMcDM]]
deps = ["Requires"]
git-tree-sha1 = "2663d058f540e0fedc19d360fbf254cadb58aa99"
...
git-tree-sha1 = "26872259612544e8eb7a17022cca7c3479552191"
