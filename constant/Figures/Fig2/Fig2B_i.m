% =========================================================================
% Fig2B_i.m
%
% PURPOSE (paper correspondence):
%   Construct circadian cosine functions for FSH, LH, E2, and P4 based on
%   experimentally measured time-of-day profiles, and overlay them with
%   normalized data points for visual comparison (Fig. 2B(i) in the paper).
%
%   The workflow is:
%     1) Load circadian sampling times and hormone concentrations.
%     2) Normalize each hormone by its mean over the circadian window.
%     3) Define a 24-hour cosine function of the form:
%           y(t) = a*cos((pi/12)*(t - c)) + d
%        where:
%           a = normalized amplitude,
%           c = acrophase (peak timing in hours),
%           d = offset (≈ 1 after normalization).
%     4) Plot the fitted cosine curves together with normalized data.
%
%   The factor (pi/12) = 2*pi/24 enforces a 24-hour period.
%
% OUTPUT:
%   Four figures showing circadian modulation of:
%     - FSH
%     - LH
%     - Estradiol (E2)
%     - Progesterone (P4)
%   Each is displayed as a normalized cosine rhythm with experimental points.
% =========================================================================

function Fig2B_i

clear all

% -------------------------------------------------------------------------
% Experimental circadian sampling times (hours) and hormone concentrations
% during the follicular phase.
% Each vector pair: time-of-day (in hours) and measured hormone value.
% -------------------------------------------------------------------------

E2timeF = [13.5, 15.6, 17.5, 19.4, 21.5, 23.5, 25.4, 27.6, 29.4, 31.5, 33.5,...
                  35.5, 37.5, 39.5, 41.4, 43.4, 45.3, 47.4, 49.3, 51.6, 53.5, 55.3];  
E2dataF = [72.8,76.1,79.4,78.8,74.9,80.4,78.2,73.2,75.3,82.5,82.5,84.1, ...
                  88.5,76.8,81.8,86.7,101,90.5,98.8,92.1,87.1,88.7];

P4timeF = [13.6, 15.6, 17.6, 19.6, 21.3, 23.6, 25.5, 27.4, 29.4, 31.4,...
           33.4, 35.3, 37.5, 39.4, 41.2, 43.2, 45.3, 47.3, 49.4, 51.3, 53.3, 55.4];   
P4dataF = [2.42,2.16,2.17,1.86,2.02,1.79,2.01,2.09,2.46,2.43,2.46,...
           2.35,2.29,2.22,2.18,2.03,2.03,1.97,2.11,2.40,2.53,2.43];

LHtimeF = [13.6, 15.7, 17.6, 19.6, 21.6, 23.6, 25.5, 27.6, 29.5,...
           31.5, 33.6, 35.6, 37.5, 39.5, 41.6, 43.6, 45.6, 47.5, 49.6, 51.6, 53.5, 55.5];
LHdataF = [5.10,5.85,5.42,5.64,4.32,4.32,4.10,4.37,4.94,4.55,...
           5.17,5.21,5.74,5.30,5.09,5.13,5.66,5.40,4.96,5.27,4.83,4.70];

FSHtimeF = [13.5, 15.5, 17.4, 19.5, 21.4, 23.5, 25.6, 27.5, 29.5,...
            31.6, 33.6, 35.4, 37.5, 39.5, 41.4, 43.5, 45.4, 47.5, 49.6, 51.5, 53.6, 55.6];   
FSHdataF = [6.29,6.43,6.46,6.80,6.15,6.15,6.01,5.81,6.25,5.84,...
            5.77,5.90,5.97,6.10,6.07,6.27,5.76,5.72,5.42,5.55,5.18,5.31];

% -------------------------------------------------------------------------
% Select circadian window (indices corresponding to one 24-h cycle)
% -------------------------------------------------------------------------

i = 10;
j = 22;

% Compute mean hormone levels over the circadian window for normalization.
meanE2 = mean(E2dataF(10:22));
meanP4 = mean(P4dataF(10:22));
meanLH = mean(LHdataF(10:22));
meanFSH = mean(FSHdataF(10:22));
 
% -------------------------------------------------------------------------
% Cosinor parameters (amplitude scaled by mean, acrophase in hours)
% These correspond to fitted values reported in the paper.
% -------------------------------------------------------------------------

a1 = 0.2739/meanLH;    c1 = 18.27;  d1 = 1;   % LH
a2 = 0.3797/meanFSH;   c2 = 16.46;  d2 = 1;   % FSH
a3 = 7.045/meanE2;    c3 = 0.5845;  d3 = 1;   % E2
a4 = 0.2303/meanP4;   c4 = 8.46;    d4 = 1;   % P4

% -------------------------------------------------------------------------
% Time vector for smooth cosine evaluation (0–60 h for plotting)
% -------------------------------------------------------------------------
 
    xdata = [0:0.1:60]';

% 24-hour cosine functions for each hormone
    cosineLH = a1*cos((pi/12)*(xdata - c1)) + d1;
    cosineFSH = a2*cos((pi/12)*(xdata - c2)) + d2;
    cosineE2 = a3*cos((pi/12)*(xdata - c3)) + d3;
    cosineP4 = a4*cos((pi/12)*(xdata - c4)) + d4;
    

% -------------------------------------------------------------------------
% Plot: FSH circadian rhythm (normalized)
% -------------------------------------------------------------------------
    figure(1)
    set(gcf, 'Color', 'white')
    p = plot(xdata, cosineFSH, 'k', 'LineWidth',1);
    hold on
    p = plot(FSHtimeF(i:j), FSHdataF(i:j)./meanFSH, 'ro', 'MarkerSize', 5,'linewidth', 3);
    ylabel('Norm $FSH$', 'Interpreter', 'latex');
    set(gca, 'Fontsize', 20)
    xlim([31, 55])
    xticks([31 55]);
    xtickangle(90);
    xticklabels({'7:00',  '7:00'});
    xlabel('t [hours]', 'Interpreter', 'latex');
    ylim([0.8, 1.2])
    yticks([0.8 1 1.2]);

% -------------------------------------------------------------------------
% Plot: LH circadian rhythm
% -------------------------------------------------------------------------
    figure(2)
    set(gcf, 'Color', 'white')
    p = plot(xdata, cosineLH, 'k', 'LineWidth',1);
    hold on
    p = plot(LHtimeF(i:j), LHdataF(i:j)./meanLH, 'ro', 'MarkerSize', 5,'linewidth', 3);
    ylabel('Norm $LH$', 'Interpreter', 'latex');
    set(gca, 'Fontsize', 20)
    xlim([31, 55])
    xticks([31 55]);
    xtickangle(90);
    xticklabels({'7:00',  '7:00'});
    xlabel('t [hours]', 'Interpreter', 'latex');
    ylim([0.8, 1.2])
    yticks([0.8 1 1.2]);

% -------------------------------------------------------------------------
% Plot: Estradiol (E2) circadian rhythm
% -------------------------------------------------------------------------
    figure(3)
    set(gcf, 'Color', 'white')
    p = plot(xdata, cosineE2, 'k', 'LineWidth',1);
    hold on
    p = plot(E2timeF(i:j), E2dataF(i:j)./meanE2, 'ro', 'MarkerSize', 5,'linewidth', 3);
    ylabel('Norm $E_2$', 'Interpreter', 'latex');
    set(gca, 'Fontsize', 20)
    xlim([31, 55])
    xticks([31 55]);
    xtickangle(90);
    xticklabels({'7:00',  '7:00'});
    xlabel('t [hours]', 'Interpreter', 'latex');
    ylim([0.8, 1.2])
    yticks([0.8 1 1.2]);
 
% -------------------------------------------------------------------------
% Plot: Progesterone (P4) circadian rhythm
% -------------------------------------------------------------------------
    figure(4)
    set(gcf, 'Color', 'white')
    p = plot(xdata, cosineP4, 'k', 'LineWidth',1);
    hold on
    p = plot(P4timeF(i:j), P4dataF(i:j)./meanP4, 'ro', 'MarkerSize', 5,'linewidth', 3);
    ylabel('Norm $P_4$', 'Interpreter', 'latex');
    set(gca, 'Fontsize', 20)
    xlim([31, 55])
    xticks([31 55]);
    xtickangle(90);
    xticklabels({'7:00',  '7:00'});
    xlabel('t [hours]', 'Interpreter', 'latex');
    ylim([0.8, 1.2])
    yticks([0.8 1 1.2]);

end

